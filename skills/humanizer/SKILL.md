---
name: humanizer
description: Use when revising prose that sounds formulaic, inflated, repetitive, vague, or chat-like while preserving the writer's meaning and factual content.
license: MIT
metadata:
  version: "2.12.0-rh1"
  adapted_for: "research-harness"
  source_commit: "e2e92e7"
---
...
## Research Harness manuscript mode

When the input comes from Research Harness, this skill is a **writing policy**, not a scientific reasoning role. Accepted Evidence, Claims, Decisions, citations, and source anchors are immutable inputs.

Protect these spans exactly unless the researcher explicitly edits them:

- citation commands and keys, including `\\cite{}` and bibliography identifiers;
- Claim, Evidence, Work, Version, Artifact, Decision, SearchRun, and Synthesis IDs;
- exact quotations, numeric values, units, equations, code, tables, links, and source anchors;
- epistemic qualifiers, population/setting/method scope, comparison conditions, and negative-evidence wording.

Return a revised **manuscript candidate**, never accepted scientific state. Include a compact semantic-change report covering removed, added, weakened, or strengthened propositions. The candidate must pass manuscript audit and human review before acceptance. If prose cannot be improved without changing a protected span or the maximum defensible wording, leave that passage unchanged and flag it.

This skill does not search literature, judge evidence quality, resolve conflicts, invent citations, or promote a candidate Claim.
...
**Preference:** Avoid repetitive or mechanical dash usage. Match the writer, venue, and project style policy. Replace a dash when another construction improves the sentence, but do not treat the punctuation mark itself as evidence of AI writing and do not change a protected span merely to remove it.
**Before:**
...
Before returning the rewrite, check whether dash usage matches the writer and the active style policy.
...
**Research Harness mode.** Return a manuscript candidate plus the semantic-change report required above. Do not overwrite accepted manuscript state directly.
...
4. Write the final version. State each point naturally instead of patching one flagged phrase at a time. If a sentence stays awkward, rewrite the paragraph around its main point. Apply the active writer, venue, or project style policy.
...
Research Harness adaptation provenance: upstream `blader/humanizer`, snapshot commit `e2e92e7`, imported as v2.11.2 and adapted locally as v2.12.0-rh1.

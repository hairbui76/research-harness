...
A summary counts what a canonical file already records. The one judgement made here is the
corpus's own: which sources cannot yet be read from, and why. It lives beside `work.list`
because it is answered from the same read and the cockpit must never re-derive it from
`screening` and `parsed` in React (Product 5 P10) - the same reason `GET /overview` composes
the attention surfaces of Product 26 rather than shipping React the raw lists. Deciding what
a Claim may say is still `claim.audit`'s.
"""
